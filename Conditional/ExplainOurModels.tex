...
\begin{algorithm}
    ...
    \begin{algorithmic}
        \Function{$\textsc{Conditional-Effect-Learn}$} {$\left( s, a, s'\right)$, \texttt{HyperGraph} $U$, $K$, $C$}
            \ForAll{predicate names $n$}
                \ForAll{predicates $p \in \Delta s$ with name $n$}
                    \State{Let $H_p$ be the hypergraph describing $p$}
                    \State{$H_n \gets merge \left( H_n, H_p \right)$}
                \EndFor
                \ForAll{predicates $q \notin \Delta s$ with name $p$}
                    \State{$C_n \gets C_n \cup cands\left( q, H_n \right)$}
                \EndFor
            \EndFor
        \EndFunction%
    \end{algorithmic}
