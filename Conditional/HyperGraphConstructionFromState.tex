...
The set of knowns can now be seen as a set of tuples, consisting of candidate tuples which have been proven to be preconditoins. 

% A more natural representation for this is simply a set of sets of vertices, each of which is a subset of a binding edge in $U$. Because of transitive connectivity property <ref>, it holds that if any two subsets of $K$ overlap, then they can be replaced by their union. A tuple $(u,v)$ proven to be a precondition can then be converted to the set $\{u,v\}$ and added to $K$, while unioning <better word for unioning> overlapping sets as described above.
...
A similar method is required for the set of proven preconditions $K$, with the added restriction that a tuple $\left( u', v' \right)$ (where $u'\in rn(u) \land v' \in rn(v)$ is only retained in $K$ if $|rn(u)| \times |rn(v)| = 1$. 
