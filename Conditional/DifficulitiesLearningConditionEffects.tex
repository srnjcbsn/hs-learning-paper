...
\begin{definition}
    An explicit state is a set of predicates where the negative predicates are explicitly mentioned. To avoid generating the infinite set, it is limited by a set of predicate names $P$ and a set of objects $\mathcal{O}$:
    \begin{equation*}
        X\left(S, P, \mathcal{O}\right) = S \cup
        \left\{ \neg p \mid 
            p\left(o_1, \dots, o_{|p|}\right) \notin s \land 
            \left\{o_1, \dots, o_{|p|} \right\} \in \mathcal{O} \land
            p \in P
        \right\}
    \end{equation*}
...
\begin{example}\label{ex:ca:light-on-3}
    Consider the last case on \figref{fig:ca:house-example}. This case is identical to the first case except it does not have a lamp, thus we learn that:
    \begin{itemize}
