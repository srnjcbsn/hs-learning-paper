...
\begin{example}\label{thm:house-example}
...
\begin{theorem}\label{thm:precondition-state}
...
\begin{example}\label{thm:sokoban-example-initial-precond}
...
\begin{example}\label{thm:house-example-2}
...
\begin{example}\label{thm:sokoban-2}
...
\begin{definition}\label{thm:binding-set}
...
And thus if ever $|B_c| = 1$ then we know that it is a correct binding because of our Invariant \ref{eq:binding_invariant} and thus it is proven.
 ...
 \begin{example}\label{thm:house-example-3}
