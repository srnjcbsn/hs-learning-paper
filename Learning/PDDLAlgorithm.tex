...
The \texttt{Result} returned by the \textit{conduct} function is used by the \textit{analyse} function to obtain new knowledge (as described in the following sections), which is then merged into the knowledge base.
Note that the generated hypothetical domain may be too restrictive for the agent to find a plan. In that case, no new knowledge can be obtained and the agent can either choose a new strategy, or query the teacher upon loop reentry.
